...
let token=jwt.sign({_id:"64641918bb35d7bd5ab79036"},"himonugupta")
...
app.get("/logout",async(req,res)=>{
  const store=req.cookies.jwt
  const verify= jwt.verify(store,"himonugupta")
  console.log(verify)
  const find =await Teacher.findOne(verify.id)
  console.log(find)
  if(find.store===verify){
...
app.get("/teachersign",async(req,res)=>{
...
res.render("teachersign",{founderror})
  ...
  if(check.emailid===emailid){
    if(check.password==password){
      ...
      token=await check.generatetoken()
